...
public class Client {
	...
	public static void main(String args[]) throws NotBoundException, IOException {
		...
		Messagerie messagerie = (Messagerie) Naming
				.lookup("//127.0.0.1:8080/chatservice");
		...
		while (true) {
			...
			if (entreeClavier[0] == "connect") {
				...
				System.out
						.print("Erreur : veuillez entrée une commande valide\n");
			}
			// Rafraichissement et affichage des nouveaux messages
			ArrayList<Map<Integer, String>> messages = messagerie
					.getMessages(nbMessages);
			nbMessages += messages.get(0).size();
			int i;
			for (i = 0; i < messages.get(0).size(); i++) {
				System.out.print(messages.get(0).get(i) + " : "
						+ messages.get(1).get(i) + "\n");
			}
		}
	}
}
